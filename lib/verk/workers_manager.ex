defmodule Verk.WorkersManager do
  ...
  def handle_call(:pause, _from, %State{status: :paused} = state), do: {:reply, :already_paused, state}
  def handle_call(:pause, _from, state), do: {:reply, :ok, state}

  def handle_call(:resume, _from, %State{status: :running} = state), do: {:reply, :already_running, state}
  ...
  def handle_info(:timeout, %State{status: :paused} = state), do: {:noreply, state}

  def handle_info(:timeout, %State{status: :pausing} = state) do
